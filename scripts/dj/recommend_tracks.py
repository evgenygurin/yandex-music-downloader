...
def recommend_track_criteria(gaps, priority='balance'):
    ...
    if gaps['sparse_bpm_ranges']:
        for bpm_range, count in gaps['sparse_bpm_ranges'].items():
            ...
            popular_keys = sorted(gaps['key_distribution'].items(),
                                 key=lambda x: x[1],
                                 reverse=True)[:5]
...
if __name__ == "__main__":
    ...
    logger.info(f"2. Скачать в dj_set_techno_2025/")
    logger.info(f"3. Запустить анализ:")
    logger.info(f"   ./run_full_analysis.sh")
    logger.info(f"4. Проверить качество:")
    logger.info(f"   python validate_playlist.py")
