...
def validate_track(track):
    ...
    return {"score": score, "issues": issues, "pass": len(issues) == 0 and score >= 60}
...
if __name__ == "__main__":
    ...
    logger.info("\n📊 Результаты:")
    logger.info(
        f"  ✅ Приняты:   {len(accepted)}/{len(original_tracks)} ({len(accepted) / len(original_tracks) * 100:.1f}%)"
    )
    logger.info(
        f"  ❌ Отклонены: {len(rejected)}/{len(original_tracks)} ({len(rejected) / len(original_tracks) * 100:.1f}%)"
    )
